...
class DUMIR:
    ...
    def __init__(self, args, data, model):
        ...
        self.lambc = args.lambc

        self.cons_criterion = InfoNCEloss(temperature=args.temperature)
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(self.train_dataloader, desc="Iteration")):
                ...
                with torch.set_grad_enabled(True):
                    ...
                    text_feats_pro = preds['text_feats']
                    audio_feats_pro = preds['audio_feats']
                    video_feats_pro = preds['video_feats']

                    cons_loss = self.cons_criterion(text_feats_pro, audio_feats_pro, video_feats_pro)

                    loss_fusion = self.criterion(preds['fusion_logist'], label_ids)
                    loss_text = self.criterion(preds['text_logist'], label_ids)
                    loss_audio = self.criterion(preds['audio_logist'], label_ids)
                    loss_video = self.criterion(preds['video_logist'], label_ids)

                    loss = loss_fusion + self.lamba*(loss_text+loss_audio+loss_video)/3 + self.lambc*cons_loss
    ...
    def _get_outputs(self, args, mode = 'eval', return_sample_results = False, show_results = False):
        ...
        for batch in tqdm(dataloader, desc="Iteration"):
            ...
            with torch.set_grad_enabled(False):
                ...
                total_logits = torch.cat((total_logits, logits['fusion_logist']))
                total_features = torch.cat((total_features, last_hiddens))
                total_labels = torch.cat((total_labels, label_ids))

                text_feats_pro = logits['text_feats']
                audio_feats_pro = logits['audio_feats']
                video_feats_pro = logits['video_feats']

                cons_loss = self.cons_criterion(text_feats_pro, audio_feats_pro, video_feats_pro)

                loss_fusion = self.criterion(logits['fusion_logist'], label_ids)
                loss_text = self.criterion(logits['text_logist'], label_ids)
                loss_audio = self.criterion(logits['audio_logist'], label_ids)
                loss_video = self.criterion(logits['video_logist'], label_ids)
