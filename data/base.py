...
class DataManager:
    ...
    def __init__(self, args, logger_name = 'Multimodal Intent Recognition'):
        ...
        self.train_data_index, self.train_label_ids = self._get_indexes_annotations(args, os.path.join(self.data_path, 'train.tsv'), args.data_mode)
        self.dev_data_index, self.dev_label_ids = self._get_indexes_annotations(args, os.path.join(self.data_path, 'dev.tsv'), args.data_mode)
        self.test_data_index, self.test_label_ids = self._get_indexes_annotations(args, os.path.join(self.data_path, 'test.tsv'), args.data_mode)
        ...
        with open(read_file_path, 'r') as f:
            ...
            for i, line in enumerate(data):
                ...
                if args.dataset in ['MIntRec']:
                    ...
                    indexes.append(index)
                    label_id = label_map[self.benchmarks['label_maps'][line[3]]]
    ...
    def _get_multimodal_data(self, args):
        ...
        mm_train_data = MMDataset(self.train_data_index, self.train_label_ids, text_data['train'], video_data['train'], audio_data['train'])
        mm_dev_data = MMDataset(self.dev_data_index, self.dev_label_ids, text_data['dev'], video_data['dev'], audio_data['dev'])
        mm_test_data = MMDataset(self.test_data_index, self.test_label_ids, text_data['test'], video_data['test'], audio_data['test'])
