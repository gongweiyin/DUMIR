...
class TextDataset:
    ...
    def _get_bert_feats(self, args, examples, base_attrs):
        ...
        if args.text_backbone.startswith('bert'):
            tokenizer = BertTokenizer.from_pretrained(args.bert_path + args.text_backbone, do_lower_case=True)
...
class DataProcessor(object):
    ...
    def _read_tsv(cls, input_file, quotechar=None):
        ...
        with open(input_file, "r") as f:
            ...
            lines = []
            for line in reader:
                if sys.version_info[0] == 2: #判断python版本是2还是3
...
class DatasetProcessor(DataProcessor):
    def __init__(self, args):
        ...
        if args.dataset in ['MIntRec']:#具体数据和标签位置的选择
            self.select_id = 3
        elif args.dataset in ['MELD', 'MIntRec2.0']:
            self.select_id = 2
